...
- Record each provider’s actual processing countries and applicable transfer mechanism, together with any required assessments. Replace the neutral transfer contact wording with verified specifics. Confirm rsms.me hosting arrangements as part of this review.
...
| Whiteboard preferences      | Legacy `TLDRAW_USER_DATA_v3` may remain from the former editor until site data is cleared. It is no longer read or written. Excalidraw fonts are served locally from `/excalidraw/fonts/`; the editor does not persist an identifier.    |
...
Repeat the inventory on production HTTPS, including actual sign-in, OAuth logout confirmation, whiteboard font loading, hosted checkout, and any edge-added storage before publishing. Provider-hosted checkout storage is governed by Polar’s notice.
